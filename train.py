...
from core.yolo3 import yolo_body
from core.loss import yolo_loss
#from keras.backend.tensorflow_backend import set_session
from core.utils import get_random_data
...
if __name__ == "__main__":
    ...
    anchors_path = 'data/anchors/coco_anchors.txt'
    # 预训练模型的位置
    ...
    # 调整非主干模型first
    if True:
        model.compile(optimizer=Adam(lr=1e-3), loss={ # 正则化
            'yolo_loss': lambda y_true, y_pred: y_pred})

        batch_size = 16
        print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
        history1 = model.fit_generator(data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes),
        ...
        print('Train 50 epochs model save in :', log_dir ,'trained_weights_stage_1.h5')
    # 出错接着训练，注释掉之前的
    #model.load_weights('logs/ep064-loss16.422-val_loss20.496.h5')
    ...
    if True:
        ...
        batch_size = 4
        print('Train on {} samples, val on {} samples, with batch size {}.'.format(num_train, num_val, batch_size))
        history2 = model.fit_generator(data_generator(lines[:num_train], batch_size, input_shape, anchors, num_classes),
                steps_per_epoch=max(1, num_train//batch_size),
                validation_data=data_generator(lines[num_train:], batch_size, input_shape, anchors, num_classes),
                validation_steps=max(1, num_val//batch_size),
                epochs=100, # !!!!
                initial_epoch=50, # !!!! 出错了这里改为当前序号
                callbacks=[logging, checkpoint])
        model.save_weights(log_dir + 'last1.h5')
        print('Train 100 epochs model save in :', log_dir ,'last1.h5')
